 ...
 *
 * <p>This class acts as a thin DAO (Data Access Object) on top of Jenkins'
 * user properties system. Each Jenkins {@link User} can have a set of
 * {@link hudson.model.UserProperty} instances attached; this class reads and writes
 * the {@link CopilotTokenUserProperty} on behalf of the calling code.
 *
 * <p>The token itself is stored as a {@link Secret}, meaning Jenkins encrypts it
 * with the master key before writing it to disk. Callers never see the raw token
 * on disk — only the encrypted form.
 *
 * <p>Lifecycle:
 * <ol>
 *   <li>💾 {@link #save} is called by {@link DeviceFlowAuthService} after a successful OAuth login.</li>
 *   <li>🔍 {@link #getToken} / {@link #getIdentity} are called by {@link CopilotClientFactory}
 *       and the auth-status endpoint whenever the token is needed.</li>
 *   <li>🗑️ {@link #delete} is called when the user clicks "Logout".</li>
 * </ol>
 */
public class GitHubTokenStore {
    ...
    /**
     * 💾 Saves the access token and identity for a Jenkins user.
     * Replaces any previously stored token for the same user.
     *
    ...
    /**
     * 🔍 Returns the stored GitHub access token for the given user, if present.
     * Returns an empty {@code Optional} when the user has never logged in.
     */
    ...
    /**
     * 🗑️ Removes the stored token and identity for the given user (logout).
     * The user will have to go through the Device Flow again to reconnect.
     */
